...
describe("deleteTestTypesList", () => {
  context(
    "database call deletes items and resolves with no unprocessed TestTypes items",
    () => {
      it("should return nothing", () => {
        ...
        return testTypesService
          .deleteTestTypesList(mockTestTypesRecords)
          .then((data) => {
      ...
      it("should return the unprocessed items on deleteTestTypesList", () => {
        ...
        return testTypesService
          .deleteTestTypesList(expectedUnprocessedTestTypesRecords)
          .then((data: any) => {
            expect(data.length).toEqual(TestTypes.length);
          });
      });
    }
  );
